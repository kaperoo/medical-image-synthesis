...
def train_diffusion(): 
    ...
    for epoch in range(EPOCHS):
        ...
        if ((epoch + 1) % 10 == 0):
           generate_and_save(f'epoch{epoch + 1}')
           filename = f"{MODEL_PATH}_diffusion_checkpoint.pth"
           torch.save(model.state_dict(), filename)
           log(f"Saved checkpoint.")
           
        if (epoch > 100 and (epoch + 1) % 10 == 0 and CLASS_EMB_WEIGHT <= 2):
           CLASS_EMB_WEIGHT += 0.25
           log(f"Class weight increased to {CLASS_EMB_WEIGHT}")
                
    torch.save(model.state_dict(), f"{MODEL_PATH}_diffusion.pth")
    log(f"Saved {MODEL_PATH}_diffusion.pth")
...
def train_discriminator():
    ...
    for epoch in range(DISCR_EPOCHS):
        for step, batch in enumerate(dataloader):
            ...
            if step % 20 == 0 or fake_images_cache is None:
            ...
            real_labels = torch.full(images.size(0), 0.9).to(DEVICE)
            fake_labels = torch.full(images.size(0), 0.1).to(DEVICE)
    ...
    torch.save(discr.state_dict(), f"{MODEL_PATH}_discr.pth")
    log(f'Saved {MODEL_PATH}_discr.pth')
...
def finetune():
    for epoch in range(FINETUNE_EPOCHS):
        for step, batch in enumerate(dataloader):
            ...
            adv_loss = get_adversarial_loss(discr, fake_images.detahc(), loss_d, DEVICE)
        ...
        if (OUTPUT_TO_FILE):
            log(f"Epoch {epoch + 1}/{EPOCHS}, Diffusion Loss: {diffusion_loss.item()}, Discr Loss: {d_loss.item()}, Adv Loss: {adv_loss.item()}, Total Loss: {total_loss.item()}")    
                    
        if ((epoch + 1) % 10 == 0):
            torch.save(model.state_dict(), f"{MODEL_PATH}_checkpoint.pth")
            torch.save(discr.state_dict(), f"{MODEL_PATH}_discr_checkpoint.pth")
...
if __name__ == "__main__":
    ...
    optimizer_d = Adam(discr.parameters(), lr=LEARNING_RATE)
    
    loss_d = nn.BCELoss()
    
    log('Training diffusion model...')
    train_diffusion()
    #model = load_model(model, f"{MODEL_PATH}_diffusion.pth")
