...
class CBAMBlock(nn.Module):
    ...
    def __init__(self, channel=1,reduction=16,kernel_size=49):
        super().__init__()
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=4, in_chans=1, num_classes=512,
                 ...
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0], image_labels=7, **kwargs):
        super().__init__()
        ...
        self.head = nn.Linear(self.num_features+self.image_labels, num_classes) if num_classes > 0 else nn.Identity()
...
class Generator(nn.Module):
    def __init__(self, z_dim, w_dim, in_channels, img_channels=1, num_classes=7):
        ...
        #self.CBAM_block = CBAMBlock(channel=in_channels)
        #self.map = MappingNetwork(z_dim+num_classes, w_dim)
        self.initial_adain1 = AdaIN(in_channels, w_dim)
        self.initial_adain2 = AdaIN(in_channels, w_dim)
        #self.initial_adain3 = AdaIN(in_channels, w_dim)
        self.initial_noise1 = injectNoise(in_channels)
        self.initial_noise2 = injectNoise(in_channels)
        #self.initial_noise3 = injectNoise(in_channels)
        self.initial_conv   = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        self.leaky          = nn.LeakyReLU(0.2, inplace=True)
        self.self_attention = Self_Attn(in_channels, 'gelu')
    ...
    def forward(self, noise, real, alpha, steps, label):
        ...
        w = self.style_swin_encoder(real, label_onehot).to(DEVICE)
        #label_onehot = F.one_hot(label, num_classes=7).float()
        #w = torch.cat((w, label_onehot), dim=1)
        ...
        #x, _ = self.self_attention(x)
       # x = self.initial_conv(x)
        #x = self.CBAM_block(x) # improvement
        #out = self.initial_adain3(self.leaky(self.initial_noise3(x)), w) # improvement
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, alpha, steps, labels):
        ...
        if steps == 0:  # i.e, image is 4x4
            ...
            #out = torch.sum(out*label_embedding, dim=1, keepdim=True) #change
            return out 
